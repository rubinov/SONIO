...
class Sound_Analyzer_App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.fig_left, (self.ax_left, self.ax_right) = plt.subplots(
            1, 2, figsize=(12, 4)
        )
        ...
        self.label_range_start = ctk.CTkLabel(
            master=self.controls_frame, text="Start time:"
        )
        self.label_range_start.grid(row=0, column=0, pady=4, padx=(0, 2))
        self.txt_start_time = ctk.CTkEntry(
            master=self.controls_frame, placeholder_text="Start time", width=120
        )
        self.txt_start_time.grid(row=0, column=1, pady=4, padx=(0, 2))

        # Label and input for ending range
        self.label_range_end = ctk.CTkLabel(
            master=self.controls_frame, text="Stop Time:"
        )
        self.label_range_end.grid(row=1, column=0, pady=4, padx=(0, 2))
        self.txt_stop_time = ctk.CTkEntry(
            master=self.controls_frame, placeholder_text="Stop time", width=120
        )
        self.txt_stop_time.grid(row=1, column=1, pady=4, padx=(0, 2))

        # Button to update the plots
        self.update_button = ctk.CTkButton(
            master=self.controls_frame, text="Update Plots", command=self.update_plots
        )
        self.update_button.grid(row=0, column=2, pady=4, padx=(0, 2))

        # Button to open the file dialog
        self.open_file_button = ctk.CTkButton(
            master=self.controls_frame, text="Open File", command=self.open_file_dialog
        )
        self.open_file_button.grid(row=2, column=2, pady=4, padx=(0, 2))
        self.file_path_label = ctk.CTkLabel(
            master=self.controls_frame, text="File path will appear here"
        )
        self.file_path_label.grid(row=2, column=0, columnspan=2, pady=4, padx=(0, 2))

        # Label to display the record file name and button
        self.rec_filename_label = ctk.CTkLabel(
            master=self.controls_frame, text="File name for recording"
        )
        self.rec_filename_label.grid(row=3, column=0, pady=4, padx=(0, 2))
        self.rec_filename_input = ctk.CTkEntry(
            master=self.controls_frame, placeholder_text="test.wav", width=240
        )
        self.rec_filename_input.grid(row=3, column=1, pady=4, padx=(0, 2))
        self.update_button = ctk.CTkButton(
            master=self.controls_frame, text="RECORD", command=self.record_file
        )
    ...
    def update_plots(self):
        ...
        if len(self.wav) > self.CHUNK:
            ...
            # update right plot
            f, t, Zxx = stft(
                self.wav, self.RATE, nperseg=500
            )  # nperseg determines the size of the window
            # Plot
            # data = np.random.rand(200, 200)
            # self.ax_right.pcolormesh(data, cmap="viridis")
            # self.ax_right.set_title("Pcolormesh")
            # self.ax_right.set_aspect("equal")
            im = self.ax_right.pcolormesh(
                t, f, np.abs(Zxx), shading="gouraud"
            )  # Use absolute value of Zxx
